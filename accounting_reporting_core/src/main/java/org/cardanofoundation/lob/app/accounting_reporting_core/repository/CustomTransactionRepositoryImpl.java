...
public class CustomTransactionRepositoryImpl implements CustomTransactionRepository {
    ...
    public List<Object[]> findAllReconciliationSpecial(ReconciliationFilterRequest body) {
        ...
        String jpql = reconciliationQuery(body.getReconciliationRejectionCode());

        Query reconciliationQuery = em.createQuery(jpql);

        reconciliationQuery.setMaxResults(body.getLimit());

        if (null != body.getPage() && 0 < body.getPage()) {
            reconciliationQuery.setFirstResult(body.getPage() * body.getLimit());
        }
    ...
    public List<Object[]> findAllReconciliationSpecialCount(ReconciliationFilterRequest body) {
        ...
        String jpql = "SELECT count(*) " +
                ...
                "WHERE r.id = (select id from accounting_reporting_core.ReconcilationEntity r_2 order by r_2.createdAt DESC limit 1)";

        String where = "";
        if (!body.getReconciliationRejectionCode().isEmpty()) {
            where += " and rv.rejectionCode in (" + body.getReconciliationRejectionCode().stream().map(Objects::toString).collect(Collectors.joining(",")) + ")";
        }
